from proxy import proxy_pyparser
...
class HTTPStream(Stream):
    ...
    def __init__(self, max_stored_messages: int = 50, max_message_size: int = 65535):
        ...
        self.current_http_message: proxy_pyparser.HttpMessage = None
        self.previous_http_messages: deque[proxy_pyparser.HttpMessage] = deque(maxlen=max_stored_messages)
    ...
    def set_current_message(self, data: bytes):
        ...
        try:            
            self.previous_http_messages.appendleft(proxy_pyparser.HttpMessageParser(self.previous_messages[0]).to_message())
            self.current_http_message = proxy_pyparser.HttpMessageParser(data).to_message()
